...
test('the voice TOOL SCHEMA is byte-identical to main — the mission mapping is instructions only', () => {
  ...
  // Re-pinned again: the CCTV area-discovery feature adds one new tool,
  // 'discover_cctv' (a real new capability — the kind of change this pin exists
  // to make loud). First-run missions still ride existing tools only.
  assert.equal(block.length, 32164, 'tool schema byte length drifted from the pinned release schema');
  assert.equal(
    crypto.createHash('sha256').update(block).digest('hex'),
    'a9dbe7acf39d4bd82e4fe5eb156c7c8f5738acd6f95a4aa46f81b83b7fc717d7',
